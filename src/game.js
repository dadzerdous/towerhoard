...
window.buyUpgrade = (type) => { 
    if (type === 'damage' && player.xp >= COST_DAMAGE) { 
        AudioMgr.playSelect(); player.xp -= COST_DAMAGE; player.stats.damage++; 
        // Safer UI update
...
function updateShopButtons() { 
    ['N','E','S','W'].forEach(dir => {
        ...
        if(btn) {
            if (state.turrets[dir]) { btn.innerText = "OWNED"; btn.className = "turret-btn owned"; } 
            else { btn.innerText = dir; btn.className = "turret-btn"; }
        }
    });
}
function updateNavLabels() { let cur = state.directions[state.dirIndex]; const getIcon = (dir) => state.turrets[dir] ? "🤖" : ""; document.getElementById('current-dir').innerHTML = cur + (state.turrets[cur] ? " <span style='font-size:20px'>🤖</span>" : ""); }
...
function triggerDamageFeedback(enemyDir) {
    ...
    if(el) { el.style.opacity = 1.0; el.style.transition = 'none'; setTimeout(() => el.style.transition = 'opacity 0.2s', 50); }
}
...
function gameLoop(timestamp) {
    ...
    // 1. RENDER BACKGROUND (Always Draw)
    renderer.clear();
    ...
    if (!isPaused) {
        if (input.update()) shoot(); // Only shoot if playing
        ...
        for (let i = state.enemies.length - 1; i >= 0; i--) {
            ...
            if (e.distance <= 0) {
                ...
                state.towerHp -= 10;
                // REMOVED FLOATING TEXT FOR TOWER/PLAYER DAMAGE
    ...
    // 3. DRAW ENEMIES (Render even if paused so they don't vanish)
    // Only draw valid enemies
    for (let i = state.enemies.length - 1; i >= 0; i--) {
        let e = state.enemies[i];
